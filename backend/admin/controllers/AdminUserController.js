...
const AdminUserRepository = require('../../models/AdminUserRepository');
...
exports.addEntity = async (req, res) => {
    try {
        ...
        const existingUser = await AdminUserResource.findByEmail(email);
        if (existingUser) {
        ...
        const newUser = await AdminUserRepository.createUser({
            firstname,
            lastname,
            email,
            role: role,
...
exports.getEntity = async (req, res) => {
    try {
        const user = await AdminUserResource.findByIdExclPassword(req.params.id);
        if (!user) {
...
exports.updateEntity = async (req, res) => {
    try {
        ...
        const user = await AdminUserResource.findByIdExclPassword(id);
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
        ...
        const updatedUser = await AdminUserRepository.updateUser(id, user);
...
exports.deleteEntity = async (req, res) => {
    try {
        ...
        const user = await AdminUserRepository.deleteUser(id);
...
exports.updateUserRole = async (req, res) => {
    try {
        ...
        const user = await AdminUserResource.findByIdAndUpdate(id, { role }, { new: true });
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
